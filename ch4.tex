...
that the asteroid belt \textbf{\cite{bottke05, morbidelli09}}, Kuiper belt \textbf{\cite{duncan89, levison08, sheppard10}} and the Oort cloud \textbf{\cite{levison11}} are largely composed of the leftovers of this stage of planet formation.
...
the gravitational interactions between the \textbf{few massive bodies produced by the runaway growth phase \cite{ida93, kokubo95, kokubo98} render statistical methods inappropriate}. Due to computational expense, N-body simulations of
...
to explain the observed architectures \textbf{\cite{izidoro17, izidoro21}}, we will focus on an in-situ
...
\noindent \textbf{Using this formalism, $e_{h}$ and $i_{h}$ describe the radial and vertical excursions of an orbiting body in units of its Hill radius. For $e_{h} > 1$, the random velocity dispersion dominates over the shearing motion across a separation of $1 r_{h}$ and encounters can be treated with a two-body formalism.}
...
protoplanets within the lifetime of a protoplanetary disk \textbf{\cite{lissauer87}, although one should note that pebble accretion \cite{lambrechts12, lambrechts14, bitsch15} is a viable alternative scenario}. Due to the
...
typically taken to be \textbf{$\approx 3$ for a planetesimal disk \cite{ida90, stewart00}}. Despite
...
\cite{kokubo98} showed that \textbf{the runaway growth process described above} is actually self-limiting. As the runaway
...
\textbf{As an additional note, the Roche limit of the central body and the distance at which $\alpha = 1$ are equivalent for a rigid spherical body. Applying a hydrostatic correction to the Roche limit, $a_{Roche} \simeq 0.6 a_{\alpha = 1}$. This accretion mode should therefore be relevant for ring systems, which is a topic that deserves further study using high resolution N-body techniques.}
...
described in \cite{richardson94} and \cite{richardson00}. To
...
cm$^{-3}$ \textbf{(for a radius of 341 km)}, and $\left< e^{2} \right>^{1/2} = 4 e_{h}$, which corresponds to $\alpha = 3.6 \times 10^{-2}$ and $\beta = 3.4 \times 10^{-3}$. These parameters are chosen to match the initial conditions of \cite{kokubo98}, which gave rise to oligarchic growth. 
...
embryos \cite{weidenschilling89, ida90}. \textbf{When $\alpha \ll 1$, the Hill sphere of a body is mostly empty space and the majority of close encounters result in viscous stirring, rather than accretion. In the opposite regime,} we 
...
\noindent where $v_{k}$ is the local Keplerian velocity \textbf{(see eq. 4.30 of \cite{armitage20})}. As in section
...
the initial mass planetesimals, the \textbf{Stokes number
...
the protoplanet assembly phase last \textbf{(as an example, see figure 1 of \cite{kokubo02}}. This radial timescale dependence is not typically
accounted for in planet formation simulations \textbf{a notable exception being \cite{emsenhuber21a, emsenhuber21b}}, and appears to be an
...
final masses of the bodies (in units of feeding zone size \textbf{$\tilde{b}$}) are shown
...
clearly visible. \textbf{The feeding zone size $\tilde{b} = 2 \sqrt{3}$ permitted by bodies on circular, non-inclined orbits \cite{naka88} is shown by the
...
top panel of figure \ref{fig:fulldisk_e_m}. \textbf{In fact, the opposite appears to be true.} There are, however, a couple of factors in the wide disk simulations that could make this 
...
planetesimals. \textbf{For the fdHi, fdHiSteep and fdHiShallow
...
\begin{center}
    ...
    \caption{\textbf{The relative separation between the final period
...
disk where planetesimals might be expected to form \textbf{\cite{mulders18}} for a wide range of $\rho_{pl}$.
...
\begin{center}
    ...
    which $\tilde{b}$ exceeds $2 \sqrt{3}$ \textbf{This is highlighted by the vertical dashed lines.}. The shaded region
...
disk, partial accretion events should allow growth to happen more quickly than what was seen in this test case \textbf{(see figure 8 of \cite{leinhardt15})}, so this growth 
...
with such a large value of $\alpha$. \textbf{Typically, it is assumed that $\alpha \ll 1$ (e.g. \cite{lithwick14}), which is certainly true for material at and beyond the Earth's orbit.} However, a value of $\alpha = 1$
...
density waves formed inside of the Roche limit and the material exterior to it\textbf{, making the dynamics driving accretion distinctly non-local, in contrast to what we have presented in this work}. \cite{ida97} was able to form 1-2
...
\textbf{As described in section \ref{sec:methods}, {\sc ChaNGa}
